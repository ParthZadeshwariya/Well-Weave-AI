...
const MeditationSpace = () => {
  ...
  const SetupView = () => (
     <div className="flex flex-col items-center justify-center h-full text-center p-8">
        ...
        <div className="flex items-center gap-2 p-3 mb-8 rounded-lg bg-primary/10 text-primary-foreground">
            <Headphones size={20} className="text-primary flex-shrink-0" />
            <span className="text-sm font-medium">For the best experience, please use headphones.</span>
        </div>

        <div className="w-full max-w-xs space-y-6">
            <div className="space-y-2">
                <label className="text-sm font-medium text-muted-foreground flex items-center justify-center gap-2"><Timer size={16}/> Duration</label>
                <Select value={String(selectedDuration)} onValueChange={(v) => setSelectedDuration(Number(v))}>
                    <SelectTrigger className="w-full bg-card/50 backdrop-blur-sm h-12 text-base"><SelectValue /></SelectTrigger>
                    <SelectContent>{durationOptions.map(o => <SelectItem key={o.value} value={String(o.value)}>{o.label}</SelectItem>)}</SelectContent>
                </Select>
            </div>
            <div className="space-y-2">
                <label className="text-sm font-medium text-muted-foreground flex items-center justify-center gap-2"><Music size={16}/> Background Music</label>
